...
def index(request):
    ...
    if request.method == "POST":
        ...
        plot_word_vector(neural_output)
...
def perform_neural_networking_train(input_vector):
    ...
    for unique_tuple in input_vector:
        ...
        lr=0.1 #Setting learning rate
        inputlayer_neurons = X.shape[0] #number of words in dictionary
        hiddenlayer_neurons = 3#100 #number of hidden layers neurons
        output_neurons = X.shape[0] #number of neurons at output layer
...
def plot_word_vector(data_matrix):
    df = pd.DataFrame(data=data_matrix, columns=["node1", "node2", "node3"])
    # plot
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(df['node1'], df['node2'], df['node3'], c='Red', s=20)
    ax.view_init(35, 195)
    plt.savefig('process/static/images/wt_plot.png')
    #return 1